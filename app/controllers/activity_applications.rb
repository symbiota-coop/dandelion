Dandelion::App.controller do
  ...
  post '/activities/:id/apply' do
    ...
    if (account = Account.find_by(email: params[:account][:email].downcase))
      @account = account
      unless @account.update_attributes(mass_assigning(Hash[params[:account].map { |k, v| [k, v] if v }.compact], Account))
  ...
  get '/activities/:id/applications', provides: %i[html csv] do
    ...
    @activity_applications = @activity_applications.and(:account_id.in => Account.and(:id.in => @activity.activity_applications.pluck(:account_id)).and(:picture_uid.ne => nil).pluck(:id)) if params[:photo]
