class Event
  ...
  def donations_to_dandelion?
    !donations_to_organisation? && organisation.donations_to_dandelion? && EventPaymentMethod.object('stripe')&.available?(self) && (suggested_donation || ticket_types.any? { |ticket_type| (ticket_type.price && ticket_type.price > 0) || ticket_type.range }) ? true : false
  end
