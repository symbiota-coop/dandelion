class Event
  ...
  before_validation do
    ...
    errors.add(:organisation, 'does not have any payment methods set up, so you can currently only issue free tickets') if ticket_types.any? { |ticket_type| ticket_type.price > 0 } && !organisation.payment_method?
  ...
  def sold_out?
    ticket_types.count > 0 && ticket_types.and(:hidden.ne => true).all? { |ticket_type| ticket_type.number_of_tickets_available_in_single_purchase < 1 }
  end
