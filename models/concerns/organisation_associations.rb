module OrganisationAssociations
  ...
  def featured_events
    primary_host_featured_events = events.live.publicly_visible.future_and_current_featured.and(has_image: true).and(featured: true)

    featured_cohostship_event_ids = cohostships.and(featured: true).pluck(:event_id)
    cohost_featured_events = Event.live.publicly_visible.future_and_current_featured.and(has_image: true).and(:id.in => featured_cohostship_event_ids)
