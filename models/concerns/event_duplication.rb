module EventDuplication
  extend ActiveSupport::Concern

  def duplicate!(account)
    event = Event.create!(
      duplicate: true,
      name: name,
      start_time: start_time,
      end_time: end_time,
      currency: currency,
      location: location,
      image: image,
      theme_color: theme_color,
      description: description,
      email: email,
      feedback_questions: feedback_questions,
      suggested_donation: suggested_donation,
      minimum_donation: minimum_donation,
      capacity: capacity,
      revenue_share_to_revenue_sharer: revenue_share_to_revenue_sharer,
      profit_share_to_organiser: profit_share_to_organiser,
      profit_share_to_coordinator: profit_share_to_coordinator,
      profit_share_to_category_steward: profit_share_to_category_steward,
      profit_share_to_social_media: profit_share_to_social_media,
      hide_attendees: hide_attendees,
      hide_discussion: hide_discussion,
      hide_from_carousels: hide_from_carousels,
      hide_few_left: hide_few_left,
      hide_organisation_footer: hide_organisation_footer,
      hide_unavailable_tickets: hide_unavailable_tickets,
      hide_waitlist: hide_waitlist,
      always_show_full_ticket_form: always_show_full_ticket_form,
      blank_price_for_free_tickets: blank_price_for_free_tickets,
      monthly_donors_only: monthly_donors_only,
      no_discounts: no_discounts,
      no_tickets_pdf: no_tickets_pdf,
      send_ticketholder_confirmation: send_ticketholder_confirmation,
      enable_resales: enable_resales,
      allow_ticket_type_waitlists: allow_ticket_type_waitlists,
      no_sales_after_end_time: no_sales_after_end_time,
      show_after_start_time: show_after_start_time,
      extra_info_for_ticket_email: extra_info_for_ticket_email,
      extra_info_for_recording_email: extra_info_for_recording_email,
      show_emails: show_emails,
      opt_in_organisation: opt_in_organisation,
      opt_in_facilitator: opt_in_facilitator,
      send_order_notifications: send_order_notifications,
      minimal_only: minimal_only,
      locked: true,
      secret: secret,
      account: account,
      last_saved_by: account,
      organisation: organisation,
      activity: activity,
      local_group: local_group,
      coordinator: coordinator,
      revenue_sharer: revenue_sharer,
      organiser: (organiser || account unless revenue_sharer),
      tag_names: event_tags.pluck(:name),
      update_tag_names: true,
      add_a_donation_to: add_a_donation_to,
      donation_text: donation_text,
      carousel_text: carousel_text,
      select_tickets_intro: select_tickets_intro,
      select_tickets_outro: select_tickets_outro,
      select_tickets_title: select_tickets_title,
      rsvp_button_text: rsvp_button_text,
      thank_you_text: thank_you_text,
      ask_hear_about: ask_hear_about,
      time_zone: time_zone,
      questions: questions,
      redirect_url: redirect_url,
      purchase_url: purchase_url,
      terms_and_conditions_url: terms_and_conditions_url,
      terms_and_conditions: terms_and_conditions,
      terms_and_conditions_check_box: terms_and_conditions_check_box,
      match_phrase: match_phrase,
      facebook_pixel_id: facebook_pixel_id,
      google_ads_conversion_id: google_ads_conversion_id,
      google_ads_conversion_label: google_ads_conversion_label,
      google_ads_enhanced_conversions: google_ads_enhanced_conversions,
      tax_rate_id: tax_rate_id,
      evergreen: evergreen,
      reminder_hours_before: reminder_hours_before,
      feedback_hours_after: feedback_hours_after,
      ticket_email_title: ticket_email_title,
      ticket_email_greeting: ticket_email_greeting,
      recording_email_title: recording_email_title,
      recording_email_greeting: recording_email_greeting,
      reminder_email_title: reminder_email_title,
      reminder_email_body: reminder_email_body,
      feedback_email_title: feedback_email_title,
      feedback_email_body: feedback_email_body
    )
    event_tags.each do |event_tag|
      event.event_tagships.create(
        event_tag: event_tag
      )
    end
    event_facilitations.each do |event_facilitation|
      event.event_facilitations.create(
        account: event_facilitation.account
      )
    end
    cohostships.each do |cohostship|
      event.cohostships.create(
        organisation: cohostship.organisation,
        image: cohostship.image
      )
    end
    ticket_groups.each do |ticket_group|
      event.ticket_groups.create(
        name: ticket_group.name,
        capacity: ticket_group.capacity
      )
    end
    ticket_types.each do |ticket_type|
      event.ticket_types.create(
        name: ticket_type.name,
        description: ticket_type.description,
        price: ticket_type.price,
        range_min: ticket_type.range_min,
        range_max: ticket_type.range_max,
        quantity: ticket_type.quantity,
        slots: ticket_type.slots,
        hidden: ticket_type.hidden,
        order: ticket_type.order,
        max_quantity_per_transaction: ticket_type.max_quantity_per_transaction,
        minimum_monthly_donation: ticket_type.minimum_monthly_donation,
        # sales_end: ticket_type.sales_end,
        ticket_group: (event.ticket_groups.find_by(name: ticket_type.ticket_group.name) if ticket_type.ticket_group)
      )
    end
    event
  end
end
