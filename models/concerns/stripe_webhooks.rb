module StripeWebhooks
  ...
  def delete_stripe_webhook
    Stripe.api_key = stripe_sk
    Stripe.api_version = '2020-08-27'
    webhook = stripe_webhooks.find { |w| w['url'] == stripe_webhook_url && w['enabled_events'].include?('checkout.session.completed') }
  ...
  def create_stripe_webhook_if_necessary
    ...
    return if stripe_webhooks.find { |w| w['url'] == stripe_webhook_url && w['enabled_events'].include?('checkout.session.completed') }
