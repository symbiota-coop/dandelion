module OrderNotifications
  ...
  included do
    ...
    handle_asynchronously :send_tickets_again
  end
  ...
  def send_tickets_again
    ...
    content = ERB.new(File.read(Padrino.root('app/views/emails/tickets.erb'))).result(binding)
    batch_message.subject(
      ('Resending with ticket PDF: ' + ((event.recording? ? event.recording_email_title : event.ticket_email_title) || (event.recording? ? event.organisation.recording_email_title : event.organisation.ticket_email_title)))
      .gsub('[ticket_or_tickets]', tickets.count == 1 ? 'Ticket' : 'Tickets')
      .gsub('[event_name]', event.name)
    )
