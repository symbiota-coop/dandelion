class Order
  ...
  before_validation do
    ...
    if affiliate_type && %w[Account Organisation].include?(affiliate_type) && affiliate_id
      resolved = affiliate_type.constantize.find(affiliate_id)
      self_assigned = resolved.is_a?(Account) && account && resolved.id == account.id
      if !resolved || self_assigned
  ...
  # Row-level read policy: your own orders, plus completed orders for events you administer
  ...
  def create_order_notification
    ...
    return unless account.publicly_visible? && event.live? && event.publicly_visible?
      ...
      orders.each do |order|
        row = [
          ...
          if event
            ...
            (order.event ? order.event.start_time : '')
          end,
          if event
            event.end_time
          else
