class Organisation
  ...
  def self.new_hints
    {
      stripe_pk: '<code>Developers</code> > <code>API keys</code> > <code>Publishable key</code>. Starts <code>pk_</code>',
  ...
  def update_paid_up
    ...
    begin
      update_attribute(:paid_up, contribution_not_required? || contribution_requested.zero? || contribution_paid >= 0.5 * contribution_requested)
    ...
    csv = CSV.generate do |csv|
      ...
      organisationships.each do |organisationship|
        csv << [
          ...
          (1 if organisationship.unsubscribed || organisationship.account.unsubscribed)
        ]
      end
    end
