...
class ApiTest < ActiveSupport::TestCase
  ...
  test 'events returns public events and hides secret and locked events' do
    ...
    ids = result['data'].map { |e| e['id'] }

    assert_equal 200, last_response.status
    assert_includes ids, @public_event.id.to_s
    refute_includes ids, @secret_event.id.to_s
    refute_includes ids, @locked_event.id.to_s
  end
  ...
  test 'get returns a single record within the resource scope' do
    ...
    get "/api/admin_events/#{@secret_event.id}"
    assert_equal 200, last_response.status
  ...
  test 'accounts returns public accounts without emails' do
    ...
    result = api_find('accounts', visible, { filter: { name: { '$in' => ['Visible Person', 'Hidden Person'] } } })

    assert_equal(['Visible Person'], result['data'].map { |a| a['name'] })
    refute result['data'].first.key?('email')
  end

  test 'my_orders and my_tickets only return your own records' do
    ...
    orders = api_find('my_orders', alice)
    tickets = api_find('my_tickets', alice)

    assert_equal([alice_order.id.to_s], orders['data'].map { |o| o['id'] })
    assert_equal([alice_order.id.to_s], tickets['data'].map { |t| t['order_id'] })

    escaped = api_find('my_orders', bob, { filter: { '$or' => [{ id: alice_order.id.to_s }, { event_id: @event.id.to_s }] } })
    refute_includes escaped['data'].map { |o| o['id'] }, alice_order.id.to_s

    api_find('my_orders', bob, { filter: { account_id: alice.id.to_s } })
    assert_equal 400, last_response.status
  end

  test 'admin_orders and admin_tickets return records for events you administer' do
    create_event(prices: [0])
    ...
    orders = api_find('admin_orders', @account)
    tickets = api_find('admin_tickets', @account, { filter: { event_id: @event.id.to_s } })
    ...
    assert_empty api_find('admin_orders', stranger)['data']
    assert_empty api_find('admin_tickets', stranger)['data']
  end

  test 'admin_orders hides emails from event admins who cannot view them' do
    ...
    orders = api_find('admin_orders', facilitator)
    tickets = api_find('admin_tickets', facilitator)
    ...
    api_find('admin_orders', facilitator, { filter: { email: attendee.email } })
    assert_equal 400, last_response.status
  end

  test 'organisation_followers returns followers of organisations you administer' do
    create_organisation
    ...
    result = api_find('organisation_followers', @account, { filter: { account_id: follower.id.to_s } })
    ...
    everything = api_find('organisation_followers', @account)
    assert(everything['data'].all? { |f| f['organisation_id'] == @organisation.id.to_s })
    assert_empty api_find('organisation_followers', follower)['data']
  end
