...
class NavTest < ActiveSupport::TestCase
  ...
  test 'header nav' do
    ...
    login_as(@account)
    visit '/'
    hrefs = all('#header .nav a', visible: :all).map { |a| a['href'] }
    hrefs.each do |href|
      puts URI(href).path
      visit href
      assert true
  ...
  test 'sidebar nav' do
    ...
    login_as(@account)
    visit '/'
    hrefs = all('#sidebar .nav a', visible: :all).map { |a| a['href'] }
    hrefs.each do |href|
      puts URI(href).path
      visit href
      assert true
  ...
  test 'organisation nav' do
    ...
    hrefs = all('#content .dropdown-menu', visible: :all)[0].all('a', visible: :all).map { |a| a['href'] }
    hrefs.each do |href|
      puts URI(href).path
      visit href
      assert true
  ...
  test 'activity nav' do
    ...
    hrefs = all('#content .dropdown-menu', visible: :all)[1].all('a', visible: :all).map { |a| a['href'] }
    hrefs.each do |href|
      puts URI(href).path
      visit href
      assert true
  ...
  test 'gathering nav' do
    ...
    hrefs = all('#gathering-nav a', visible: :all).map { |a| a['href'] }
    hrefs.each do |href|
      puts URI(href).path
      visit href
      assert true
