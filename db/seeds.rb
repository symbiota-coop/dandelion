...
@gathering = FactoryBot.create(:gathering, account: @account, name: 'Test Gathering', slug: 'test-gathering')
@organisation = FactoryBot.create(:organisation, account: @account, name: 'Test Organisation', slug: 'test-organisation', paid_up: true)
@event = FactoryBot.create_list(:event, 4, organisation: @organisation, account: @account, last_saved_by: @account, image: File.open(Padrino.root('app/assets/images/test-event.jpg')), featured: true)
